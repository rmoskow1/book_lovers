...
class UserAPIViewTest(TestCase):
    ...
    def setUp(self):
        self.detail_view = UserViewSet.as_view(actions={'get': 'retrieve'})
        self.list_view = UserViewSet.as_view(actions={'get': 'list', 'post': 'create'})
    ...
    def test_passwords(
        ...
        new_user = UserSerializer().create(data)
        self.assertNotEqual(new_user.password,
                            data.get('password'))  # the password input is not saved as it's original string

        self.assertIsNotNone(new_user.password)  # user's password is not none
        self.assertTrue(is_password_usable(new_user.password))  # checks that a password appears hashed
        self.assertTrue(check_password(data.get('password'),
                                       new_user.password))  # confirms second param as proper hashing of first
